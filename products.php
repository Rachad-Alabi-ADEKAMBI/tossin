                            ...
                            <div v-for="entry in history" :key="entry.id" class="border rounded-lg p-4 hover:bg-gray-50">
                                ...
                                <div class="flex justify-between items-start">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <!-- Ajout du stock avant et après l'opération -->

                                        <div class="flex items-center space-x-4 mb-2">

                                            <span class="text-sm text-gray-600">

                                                <i class="fas fa-arrow-left mr-1"></i>Stock avant: <strong class="text-gray-900">{{ formatNumber(entry.stock_before) }}</strong>

                                            </span>

                                            <span class="text-sm text-gray-600">

                                                <i class="fas fa-arrow-right mr-1"></i>Stock après: <strong class="text-gray-900">{{ formatNumber(entry.stock_after) }}</strong>
        ...
        createApp({
            ...
            methods: {
                ...
                async adjustStock() {
                    ...
                    formData.append('stock_before', this.selectedProduct.quantity);
                ...
                async viewHistory(product) {
                    ...
                    try {
                        ...
                        rawHistory.sort((a, b) => new Date(a.created_at) - new Date(b.created_at));



                        // Calculer le stock avant et après pour chaque entrée

                        let currentStock = product.quantity;



                        // Parcourir l'historique en sens inverse pour calculer

                        for (let i = rawHistory.length - 1; i >= 0; i--) {

                            const entry = rawHistory[i];

                            entry.stock_after = currentStock;

                            entry.stock_before = currentStock - parseFloat(entry.quantity);

                            currentStock = entry.stock_before;
                        ...
                        this.history = rawHistory.reverse();



                        // Debug : afficher les données reçues

                        console.log('Historique calculé :', JSON.parse(JSON.stringify(this.history)));
                    ...
                    this.history.forEach(entry => {
                        ...
                        historyRows += `
                            ...
                            <tr>
                                ...
                                <td style="text-align: center;">${this.formatNumber(entry.stock_before)}</td>

                                <td style="text-align: center;">${this.formatNumber(entry.stock_after)}</td>
